...
// ── Handlers ──────────────────────────────────────────────────────────────────
/** health_get_office_hours */
export async function getOfficeHours(params, startMs) {
    ...
    if (params.day) {
        ...
        hours = hours.filter((h) => h.day === dayLower);
    }
    if (params.location_id) {
        // Fetch facility name to attach location context
        const raw = await openemr.get(`/api/facility/${params.location_id}`);
        const fac = extractList(raw)[0];
...
/** health_get_location */
export async function getLocation(params, startMs) {
    const limit = params.limit && params.limit > 0 ? params.limit : 20;
    let raw;
    if (params.location_id) {
        raw = await openemr.get(`/api/facility/${params.location_id}`);
    }
    else {
        raw = await openemr.get('/api/facility');
    }
    let facilities = extractList(raw);
    if (params.query) {
        const q = params.query.toLowerCase();
        facilities = facilities.filter((f) => (f.name ?? '').toLowerCase().includes(q) ||
...
/** health_get_billing_policy */
export async function getBillingPolicy(params, startMs) {
...
/** health_get_procedure_catalog */
export async function getProcedureCatalog(params, startMs) {
    const limit = params.limit && params.limit > 0 ? params.limit : 20;
    const raw = await openemr.get('/api/list/appttype');
    let categories = extractList(raw);
    if (params.procedure_code) {
        categories = categories.filter((c) => String(c.pc_catid ?? '') === params.procedure_code);
    }
    if (params.query) {
        const q = params.query.toLowerCase();
        categories = categories.filter((c) => (c.pc_catname ?? '').toLowerCase().includes(q) ||
