...
export async function verifyInsurance(
  params: z.infer<typeof verifyInsuranceSchema>,
  startMs: number,
): Promise<CanonicalResponse> {
  const apiKey = process.env['STEDI_API_KEY'];
  const endpoint =
    process.env['STEDI_ENDPOINT'] ??
    'https://healthcare.us.stedi.com/2024-04-01/change/medicalnetwork/eligibility/v3';

  if (!apiKey) {
    return preconditionFailed(
      'health_verify_insurance',
      'STEDI_API_KEY is not configured. Set this environment variable to enable insurance verification.',
      startMs,
    );
  }
  ...
  if (params.stedi_request) {
    body = params.stedi_request as Record<string, unknown>;
  } else {
    // Validate minimum required fields
    const missing: string[] = [];
    if (!params.provider_npi) missing.push('provider_npi');
    if (!params.member_id) missing.push('member_id');
    if (!params.first_name) missing.push('first_name');
    if (!params.last_name) missing.push('last_name');
    if (!params.date_of_birth) missing.push('date_of_birth');

    if (missing.length > 0) {
      return invalidRequest(
        'health_verify_insurance',
        `Missing required fields when stedi_request is not provided: ${missing.join(', ')}`,
        startMs,
        missing.map((f) => ({ field: f, issue: 'required' })),
      );
    }
  ...
  if (!response.ok) {
    ...
    return providerError(
      'health_verify_insurance',
      `Stedi eligibility check failed (${response.status}): ${text}`,
      startMs,
      response.status >= 500,
    );
  }
  ...
  return success('health_verify_insurance', {
    ...
    disclaimer:
      'Insurance eligibility responses indicate benefit information at time of inquiry and do not guarantee coverage or payment.',
  }, startMs);
}
